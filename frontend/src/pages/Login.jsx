...
const schema = yup.object({
  usernameOrEmail: yup.string().required('Username or Email is required').min(3, 'Must be at least 3 characters'),
...
const Login = () => {
  ...
  // 🔥 AUTH GUARD — prevents entering login when already logged in
  useEffect(() => {
      ...
      <div className="login-card">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="login-form" autoComplete="off">
          <div className="form-group-custom">
            <label htmlFor="usernameOrEmail" className="form-label-custom">Username or Email</label>
            <input
              id="usernameOrEmail"
              type="text"
        ...
        <div className="form-links-container">
          ...
          <Link to="/request-password-reset" className="forgot-password-link">Forgot Password?</Link>
        </div>
      </div>
    </div>
  );
